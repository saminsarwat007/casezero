...
## Frame 3 — The case moves

- scene: The real Boardroom film runs full and large while three plain-language proof lines arrive on their spoken cues.
- voiceover: "From there, the right information comes together. Records are checked. The deadline is watched. And the next step is prepared."
- duration: 10.344s
- poster: 10.2s
- transition_in: blur-crossfade
- status: outline
- src: compositions/frames/03-case-moves.html
- type: feature_showcase
- persuasion: Show-don't-tell proof
- beat: clarity + momentum
- blueprint: video-text-pivot (Adapt)
- asset_candidates: assets/Axiom-Boardroom-Demo.mp4 — real CaseZero product film following a complaint through the Boardroom; assets/boardroom-complete.png — real completed Boardroom state
- focal: assets/Axiom-Boardroom-Demo.mp4
- roles: Axiom-Boardroom-Demo.mp4 = background; boardroom-complete.png = supporting
- sfx: none

narrativeRole: Prove that the complaint visibly advances without explaining technical architecture.
keyMessage: CaseZero turns a submitted complaint into visible, ordered work.

Adapt: keep the show→yield→stamp logic, but the real product video remains the hero; the “impact stat” becomes three plain-language receipts rather than invented metrics.
Scene 1 (0.0–2.6s): The muted real Boardroom film fills the canvas inside a thin bone keyline; no browser chrome, no headline yet — smooth video establish and a single measured push that stops before the back half.
Scene 2 (2.6–4.8s): “Records are checked.” arrives on a bone label at the left edge while the live case film continues; the phrase enters alone with a long-tail settle (`dynamic-content-sequencing`).
Scene 3 (4.8–7.1s): “Deadline watched.” replaces the first receipt in the same anchor as the film reaches its evidence chain; hard text swap (`discrete-text-sequence`) with no camera drift.
Scene 4 (7.1–9.0s): “Next step ready.” replaces the receipt in burgundy as the completed Boardroom still briefly scale-swaps over the corresponding region (`scale-swap-transition`).
Scene 5 (9.0–10.344s): Hold on the full completed Boardroom with the final receipt small at left; stillness closes the proof.
