# Submission Proof Register

Store genuine evidence here. Scratch exports belong in `proof/scratch/` and are
ignored. Do not create simulated or edited proof images.

## Naming

`YYYY-MM-DD_HHMM_product_session_page.ext`

Examples:

- `2026-08-04_1410_codebuddy_s2_p1.png`
- `2026-08-04_1515_workbuddy_email_mcp_run.png`
- `2026-08-05_1030_casezero_live_smoke.txt`

## Required owner-supplied evidence

- [ ] Full CodeBuddy conversation-history export.
- [ ] 10+ dated CodeBuddy/WorkBuddy screenshots (absolute minimum: 3).
- [ ] WorkBuddy agent configuration using Email MCP and the PDF skill.
- [ ] WorkBuddy run showing the request accepted by `/intake/workbuddy`.
- [x] Final public URL and public Playwright result.
- [x] 380×216 cover image in `submission/03-cover/`.
- [x] Final 5:55.77 narrated stakeholder film with captions and chapter QA.

## Repository-produced evidence

- [x] Full backend acceptance suite: 501 passing tests.
- [x] Twenty-nine-journey Playwright stakeholder acceptance suite.
- [x] Real MCP stdio smoke.
- [x] Gemini/Groq/vision smoke.
- [x] Live WorkBuddy HTTP intake smoke.
- [x] Supabase append-only/tamper proof.
- [x] Persisted production evaluation.
- [x] FMOS pack render and visual QA.
- [x] Motion and web-interface audits.

## Final stakeholder film

- Public product: <https://casezero-alpha.vercel.app>
- Upload-ready MP4: `submission/04-demo/CaseZero-Stakeholder-Demo.mp4`
- Transcript: `submission/04-demo/CaseZero-Stakeholder-Demo.en.vtt`
- Capture: 5:55.77, 1,920×1,080, 25fps, H.264/AAC with fast-start metadata.
- Sequence: case-study stakes → six-agent/kernel boundary → onboarding → Simple →
  Mission Control → Axiom → customer resolution → audit/quarantine → Settings →
  work-email invitation → measured proof → controlled rollout.
