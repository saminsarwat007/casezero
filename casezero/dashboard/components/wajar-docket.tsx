...
export function WajarDocket() {
  ...
  async function prepare(event?: FormEvent) {
    ...
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Axiom could not prepare this action."); }
  ...
  async function continueAction() {
    ...
    try {
      if (isRehearsal()) {
        setReceipt(`AXR-${plan.plan_id.replace("AXP-", "")}-REHEARSAL`);
      } else {
    ...
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Axiom could not execute this action."); }
    finally { setBusy(false); }
  }

  return (
    <>
      <button className="wajar-launch" type="button" onClick={() => setOpen(true)} aria-label="Open Axiom operating agent" aria-expanded={open} aria-controls="wajar-docket">
      ...
      {open ? <div className="wajar-scrim" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
        <aside className="wajar-docket" id="wajar-docket" role="dialog" aria-modal="true" aria-labelledby="wajar-title">
          <header className="wajar-head">
            ...
            <button className="wajar-close" type="button" onClick={() => setOpen(false)} aria-label="Close Axiom">×</button>
          </header>
          ...
          {!plan ? <div className="wajar-prompts" aria-label="Suggested Axiom commands">{prompts.map((prompt) => <button type="button" key={prompt} onClick={() => { setCommand(prompt); setPlan(null); inputRef.current?.focus(); }}>{prompt}</button>)}</div> : null}
          ...
          {plan ? <section className="action-docket" aria-live="polite">
            ...
            {receipt ? <div className="wajar-receipt" role="status"><span>Execution receipt</span><strong className="mono">{receipt}</strong><small>{isRehearsal() ? "Rehearsal only · no live control changed" : "Recorded in the Axiom action ledger"}</small></div> : null}
