      ...
      <form className="settings-layout section" onSubmit={save}>
        <div className="settings-sheet panel">
          ...
          <div className="settings-fields">
            <div className="field"><label htmlFor="bank-name">Bank display name</label><input className="input" id="bank-name" name="bank_display_name" autoComplete="organization" value={controls.bank_display_name} onChange={(event) => change("bank_display_name", event.target.value)} minLength={2} maxLength={120} required /><p className="field-help">Used in the workspace masthead and staff-facing documents.</p></div>
            <div className="field"><label htmlFor="complaints-email">Complaints contact email</label><input className="input" id="complaints-email" name="complaints_email" type="email" autoComplete="email" spellCheck={false} value={controls.complaints_email} onChange={(event) => change("complaints_email", event.target.value)} required /><p className="field-help">Printed on customer and FMOS communications. Inbox credentials remain deployment secrets.</p></div>
            <div className="settings-pair">
              <div className="field"><label htmlFor="timezone">Operating timezone</label><select className="select" id="timezone" name="timezone" autoComplete="off" value={controls.timezone} onChange={(event) => change("timezone", event.target.value as Controls["timezone"])}><option value="Asia/Kuala_Lumpur">Malaysia time · UTC+8</option><option value="UTC">UTC</option></select></div>
              <div className="field"><label htmlFor="default-workspace">Default workspace</label><select className="select" id="default-workspace" name="default_workspace" autoComplete="off" value={controls.default_workspace} onChange={(event) => change("default_workspace", event.target.value as Controls["default_workspace"])}><option value="/simple">Today · Simple</option><option value="/pro">Mission control · Pro</option></select></div>
            </div>
            <div className="field"><label htmlFor="warning-hours">SLA warning horizon</label><div className="unit-input"><input className="input" id="warning-hours" name="sla_warning_hours" type="number" inputMode="numeric" autoComplete="off" min={1} max={120} value={controls.sla_warning_hours} onChange={(event) => change("sla_warning_hours", Number(event.target.value))} /><span className="mono">HOURS</span></div><p className="field-help">Axiom and Mission Control surface open cases due inside this horizon. BNM working-day deadlines themselves do not change here.</p></div>
          </div>
        </div>
        ...
        <aside className="settings-controls">
          <div className="panel control-card">
            ...
            <label className="control-toggle"><span><strong>Axiom operating agent</strong><small>Allow signed-in staff to prepare read, navigation, and governed Admin actions.</small></span><input name="wajar_enabled" type="checkbox" checked={controls.wajar_enabled} onChange={(event) => change("wajar_enabled", event.target.checked)} /><i aria-hidden="true" /></label>
            <label className="control-toggle risk"><span><strong>Automatic financial resolution</strong><small>When off, otherwise eligible PASS cases stop at human review. It never expands a pack threshold.</small></span><input name="automatic_resolution_enabled" type="checkbox" checked={controls.automatic_resolution_enabled} onChange={(event) => change("automatic_resolution_enabled", event.target.checked)} /><i aria-hidden="true" /></label>
          </div>
