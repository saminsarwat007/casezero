        ...
        <div className="customer-sheet proactive-sheet">
          ...
          {!alert ? <div className="skeleton section" role="status" aria-label="Loading transaction alert" /> : result?.case?.outcome === "RESOLVED_IN_FULL" ? (
            ...
            <>
              ...
              <section className="proactive-actions" aria-label="Confirm transaction"><button className="btn ghost" disabled={busy} onClick={() => void decide(false)}>Yes, it was me</button><button className="btn danger" disabled={busy} onClick={() => void decide(true)}>{busy ? "Filing dispute…" : "Not me — dispute it"}</button></section>
