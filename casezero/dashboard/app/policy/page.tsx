    ...
    <AppShell>
      ...
      <div className="detail-grid section">
        <section className="panel panel-pad">
          ...
          <div className="field" style={{ marginTop: 14 }}><label htmlFor="policy-request">Policy instruction</label><textarea id="policy-request" name="policy_request" autoComplete="off" className="textarea" value={request} onChange={(event) => setRequest(event.target.value)} /><span className="field-help">Protected policy paths are refused before simulation.</span></div>
          <button className="btn primary" disabled={busy || request.trim().length < 8} style={{ marginTop: 14 }} onClick={() => void compose()}>{busy ? "Running 200-case replay…" : "Interpret and simulate"}</button>
